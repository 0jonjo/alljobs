...
describe 'User applied to job opening' do
  ...
  it "and see multiply applies" do
    job1 = Job.create!(title: 'Job Opening Test 123', description: 'Lorem ipsum dolor sit amet', 
                      skills: 'Nam mattis, felis ut adipiscing.', salary: '99', 
                      company: 'Acme', level: 'Junior', place: 'Remote Job',
                      date: 24/12/2099)
    job2 = Job.create!(title: 'Job Opening 2', description: 'Lorem ipsum dolor sit amet', 
                      skills: 'Test.', salary: '999', 
                      company: 'Acme', level: 'Junior', place: 'Remote Job',
                      date: 24/12/2099)
    user = User.create!(:email => 'user@test.com', :password => 'test123')
    apply1 = Apply.create!(:job => job1, :user => user)
    apply1 = Apply.create!(:job => job2, :user => user)
    login_as(user, :scope => :user)

    visit applies_path
    expect(page).to have_content('Job Opening Test 123')
    expect(page).to have_content('Job Opening 2')
    expect(page).to have_content('Apply ID')
  end
